...
class SearchSupport:
    ...
    def _build_search_entities(self, query: str, query_state: dict[str, Any], missing_slots: list[Any]) -> list[str]:
        candidates: list[str] = []
    ...
    @staticmethod
    def _unpack_evidence_entries_result(result: Any) -> tuple[list[dict[str, str]], Optional[list[Any]], dict[str, Any]]:
        """
    ...
    def _merge_ledger(self, current: list[dict[str, str]], new_entries: list[dict[str, str]]) -> list[dict[str, str]]:
        if not new_entries:
            return list(current)
        new_slot_keys = {self._normalize_slot(entry.get('slot', '')) for entry in new_entries if self._normalize_slot(entry.get('slot', ''))}
        merged = [entry for entry in current if self._normalize_slot(entry.get('slot', '')) not in new_slot_keys]
        seen = set()
        for entry in new_entries:
            key = (self._normalize_slot(entry.get('slot', '')), str(entry.get('value', '') or ''), str(entry.get('citation', '') or ''))
    ...
    def _build_tool_search_plan(self, state: AgentState, routed_tool: str, routed_args: dict[str, Any], loop_state: Any, top_k: int) -> ToolSearchPlan:
        answer_type = str(state.query_state.get('answer_type', '')).lower()
        missing_count = len(state.missing_slots) if isinstance(state.missing_slots, list) else 0
        base_entities = self._build_search_entities(state.user_query, state.query_state, state.missing_slots)
        ...
        if loop_state.consecutive_no_progress > 0 and state.missing_slots:
            ...
            entities = self._build_search_entities('', state.query_state, focus_missing_slots) + entities
        ...
        return ToolSearchPlan(entities=entities, search_entities=search_entities, depth=depth, top_k=top_k)

    @staticmethod
    def _update_consecutive_no_progress(loop_state: Any, new_entries_count: int, missing_slots: list[Any]) -> None:
    ...
    async def _handle_retrieval_tool_call(self, *, state: AgentState, turn: int, tool_call_id: str, routed_tool: str, routed_args: dict[str, Any], loop_state: Any, top_k: int, started: float) -> None:
        ...
        state.missing_slots = self._resolve_missing_slots(state.query_state, state.evidence_ledger, model_missing_slots=model_missing_slots)
        self._update_consecutive_no_progress(loop_state=loop_state, new_entries_count=len(new_entries), missing_slots=state.missing_slots)
        await self._refresh_context_and_slots(state, trace_step=f'context_refresh_{turn}')
        append_trace(state.trace, step=f'ledger_update_{turn}', input={'entities': plan.entities, 'depth': plan.depth, 'top_k': plan.top_k}, output={'retrieved_raw': data_raw_count, 'retrieved': len(data), 'entity_filter': filter_diag, 'new_entries': len(new_entries), 'ledger_size_before': prev_len, 'ledger_size_after': len(state.evidence_ledger), 'model_missing_slots': model_missing_slots, 'ledger_debug': ledger_debug, 'missing_slots': state.missing_slots, 'context_chars': len(state.context)}, duration_ms=(time.perf_counter() - started) * 1000.0)
    ...
    def _build_bootstrap_entities(self, state: AgentState) -> list[str]:
        ...
        search_entities = self._build_search_entities(state.user_query, state.query_state, state.missing_slots)
        missing_count = len(state.missing_slots) if isinstance(state.missing_slots, list) else 0
    ...
    async def _bootstrap_hybrid_search(self, state: AgentState, loop_state: Any) -> None:
        ...
        bootstrap_entries, bootstrap_missing_slots, bootstrap_ledger_debug = self._unpack_evidence_entries_result(bootstrap_result)
        state.evidence_ledger = self._merge_ledger(state.evidence_ledger, bootstrap_entries)
        state.ledger_attempts.append({'step': 'bootstrap_retrieval', 'retrieved': len(bootstrap_data), 'diagnostics': bootstrap_ledger_debug})
        state.missing_slots = self._resolve_missing_slots(state.query_state, state.evidence_ledger, model_missing_slots=bootstrap_missing_slots)
        await self._refresh_context_and_slots(state, trace_step='context_refresh_bootstrap')
        append_trace(state.trace, step='bootstrap_retrieval', input={'entities': bootstrap_entities, 'depth': 1, 'top_k': bootstrap_top_k}, output={'retrieved_raw': bootstrap_data_raw, 'retrieved': len(bootstrap_data), 'entity_filter': bootstrap_filter_diag, 'new_entries': len(bootstrap_entries), 'model_missing_slots': bootstrap_missing_slots, 'ledger_debug': bootstrap_ledger_debug, 'missing_slots': state.missing_slots, 'context_chars': len(state.context)}, duration_ms=(time.perf_counter() - started) * 1000.0)

class ToolCallsSupport:

    def _route_tool_call(self, requested_tool: str, args: dict[str, Any], state: AgentState) -> tuple:
        ...
        if requested_tool == 'calculator':
            return ('calculator', {'expression': expression, 'precision': precision}, 'calculator_default')
    ...
    def _parse_tool_call_args(self, tool_call: Any) -> dict[str, Any]:
        try:
            ...
            logger.warning('Failed to parse tool args for %s: %s', str(tool_call.function.name), tool_call.function.arguments)
            return {}
    ...
    def _extract_textual_tool_calls(response_text: str) -> list[dict[str, Any]]:
        ...
        if not response_text:
            return calls
        for raw_json in re.findall('<tool_call>\\s*(\\{.*?\\})\\s*</tool_call>', response_text, flags=re.DOTALL):
            try:
    ...
    async def _handle_calculator_tool_call(self, *, state: AgentState, turn: int, tool_call_id: str, routed_args: dict[str, Any], loop_state: Any, started: float) -> None:
        calc = self._call_calculator(expression=routed_args.get('expression', ''), precision=routed_args.get('precision', None))
        loop_state.tool_calls_used += 1
        tool_content = ''
        if calc.get('ok'):
            tool_content = f"Calculator expression: {calc.get('expression')}\nCalculator result: {calc.get('result')}"
            calc_chunk = {'title': 'CALCULATOR', 'page': 0, 'sent_id': -100000 - turn - loop_state.tool_calls_used, 'text': tool_content}
            state.all_context_data.append(calc_chunk)
        else:
            tool_content = f"Calculator error for expression '{routed_args.get('expression', '')}': {calc.get('error', 'unknown error')}"
        state.history.append({'role': 'tool', 'tool_call_id': tool_call_id, 'name': 'calculator', 'content': tool_content})
        await self._refresh_context_and_slots(state, trace_step=f'context_refresh_{turn}')
        append_trace(state.trace, step=f'calculator_update_{turn}', input={'expression': routed_args.get('expression', ''), 'precision': routed_args.get('precision', None)}, output={'ok': bool(calc.get('ok', False)), 'result': calc.get('result', ''), 'error': calc.get('error', ''), 'missing_slots': state.missing_slots, 'context_chars': len(state.context)}, duration_ms=(time.perf_counter() - started) * 1000.0)

    async def _handle_tool_call_response(self, state: AgentState, turn: int, resp: Any, loop_state: Any) -> None:
        ...
        for idx, tc in enumerate(tool_calls_to_process):
            ...
            logger.info('Tool routing: requested=%s -> routed=%s (%s)', t_name, routed_tool, route_reason)
    ...
    async def _handle_textual_tool_call_response(self, state: AgentState, turn: int, response_text: str, loop_state: Any) -> bool:
        tool_calls = self._extract_textual_tool_calls(response_text)
        if not tool_calls:
            return False
        append_trace(state.trace, step=f'textual_tool_calls_parsed_{turn}', input=response_text, output={'count': len(tool_calls)})
        remaining_budget = max(loop_state.max_tool_calls - loop_state.tool_calls_used, 0)
        if remaining_budget <= 0:
            append_trace(state.trace, step=f'tool_calls_truncated_{turn}', input={'received_tool_calls': len(tool_calls), 'processed_tool_calls': 0, 'max_tool_calls': loop_state.max_tool_calls, 'tool_calls_used': loop_state.tool_calls_used}, output={'reason': 'tool_call_budget_exhausted'})
            return True
        for idx, tool_call in enumerate(tool_calls[:remaining_budget]):
            ...
            routed_tool, routed_args, route_reason = self._route_tool_call(t_name, args, state)
            logger.info('Textual tool routing: requested=%s -> routed=%s (%s)', t_name, routed_tool, route_reason)
            if routed_tool == 'calculator':
                await self._handle_calculator_tool_call(state=state, turn=turn, tool_call_id=tool_call_id, routed_args=routed_args, loop_state=loop_state, started=started)
                continue
            await self._handle_retrieval_tool_call(state=state, turn=turn, tool_call_id=tool_call_id, routed_tool=routed_tool, routed_args=routed_args, loop_state=loop_state, top_k=top_k, started=started)
