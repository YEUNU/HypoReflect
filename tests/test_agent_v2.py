...
async def test_agent_5_stage_loop_and_strict_reflection():
    ...
    service._run_refinement_loop = AsyncMock(return_value=True)  # noqa: SLF001

    # Keep focus on loop orchestration; disable post-refinement overrides.
    service.execution._build_operating_margin_driver_override_answer = MagicMock(return_value=None)  # noqa: SLF001
    service.execution._build_segment_drag_override_answer = MagicMock(return_value=None)  # noqa: SLF001
    service.execution._build_debt_securities_override_answer = MagicMock(return_value=None)  # noqa: SLF001
    service.execution._build_quick_ratio_health_override_answer = MagicMock(return_value=None)  # noqa: SLF001
    service.execution._build_capital_intensity_override_answer = MagicMock(return_value=None)  # noqa: SLF001
    service.execution._build_dividend_stability_override_answer = MagicMock(return_value=None)  # noqa: SLF001
    ...
    service._run_refinement_loop.assert_awaited_once()  # noqa: SLF001
...
async def test_agent_strict_reflection_failure_and_refinement():
    ...
    service._run_refinement_loop = AsyncMock(side_effect=_mock_refinement_loop)  # noqa: SLF001
